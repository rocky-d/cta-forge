...
async def main():
    ...
    print("=" * 60, flush=True)
    print(
        "CTA-Forge v11 — Expanded Universe Max-Range Backtest",
        flush=True,
    )
    print("=" * 60, flush=True)
    ...
    fig.autofmt_xdate()
    fig.savefig(
        OUT_DIR / "backtest_v11_expanded.png",
        dpi=150,
        bbox_inches="tight",
        facecolor="white",
    )
    ...
    # Save metrics
    (OUT_DIR / "metrics_v11_expanded.json").write_text(
        json.dumps(
            {
    ...
    print(
        f"Charts: {OUT_DIR / 'backtest_v11_expanded.png'}",
        flush=True,
    )
