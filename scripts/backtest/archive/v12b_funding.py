            ...
            for adx_t in adx_ens:
                ...
                if (
                    ...
                    if 0 <= li < len(fr_d["zscore"]):
                        ...
                        if abs(fc) > 0.008:  # 0.5% cumulative over 7.5 days
                            if fc > 0 and raw > 0:
...
async def main():
    ...
    fig.autofmt_xdate()
    fig.savefig(
        OUT_DIR / "backtest_v12b_funding.png",
        dpi=150,
        bbox_inches="tight",
        facecolor="white",
    )
    ...
    # Save metrics
    (OUT_DIR / "metrics_v12b_funding.json").write_text(
        json.dumps(
            {
    ...
    print(
        f"Charts: {OUT_DIR / 'backtest_v12b_funding.png'}",
        flush=True,
    )
