"""CTA-Forge v12 — v10g + Funding Rate factor."""
...
import polars as pl
    ...
    0, "/home/node/.openclaw/workspace/cta-forge-dev/libs/cta-core/src"
)
    0, "/home/node/.openclaw/workspace/cta-forge-dev/services/reporter/src"
)
...
FR_DIR = Path("/home/node/.openclaw/workspace/cta-forge-dev/data/funding-rate")


def load_funding_rates():
    """Load funding rate CSVs into {symbol: [(timestamp_ms, rate), ...]}."""
    fr_data = {}
    for f in FR_DIR.glob("*.csv"):
        sym = f.stem
        rows = []
        with open(f) as fh:
...
def build_fr_lookup(fr_data, bars_dict):
    """Build per-symbol funding rate arrays aligned to bar timestamps.

    For each bar, use the most recent funding rate before that bar's time.
    Also compute a rolling z-score of funding rate (20-period).
    """
    fr_aligned = {}
    for sym, df in bars_dict.items():
        if sym not in fr_data:
            continue
        timestamps = [
            int(t.timestamp() * 1000) for t in df["open_time"].to_list()
        ]
        fr_rows = fr_data[sym]
        fr_ts = [r[0] for r in fr_rows]
        fr_vals = [r[1] for r in fr_rows]
        n = len(timestamps)
        rates = np.zeros(n)
        for i, bar_ts in enumerate(timestamps):
            idx = bisect_right(fr_ts, bar_ts) - 1
            if idx >= 0:
                rates[i] = fr_vals[idx]

        # Rolling z-score (60 periods ≈ 15 days at 6h bars)
        zscore = np.zeros(n)
        window = 60
        for i in range(window, n):
            chunk = rates[i - window : i]
            mu = np.mean(chunk)
            sigma = np.std(chunk)
            if sigma > 1e-10:
                zscore[i] = (rates[i] - mu) / sigma

        # Cumulative funding (sum of last 30 bars ≈ 7.5 days)
        cum_fr = np.zeros(n)
        cum_window = 30
        for i in range(cum_window, n):
            cum_fr[i] = np.sum(rates[i - cum_window : i])
...
def compute_signals(data, timeline, params, fr_aligned=None):
    ...
    fr_weight = params.get("funding_rate_weight", 0.0)
    fr_zscore_threshold = params.get("fr_zscore_threshold", 2.0)
        ...
        for gt in range(start + max(MOM_LBS) + 1, start + n):
            ...
            for adx_t in adx_ens:
                ...
                if (
                    fr_weight > 0
                    and fr_aligned
                    and sym in fr_aligned
                ):
                    fr_d = fr_aligned[sym]
                    li = gt - d["start_idx"]
                    if 0 <= li < len(fr_d["zscore"]):
                        fz = fr_d["zscore"][li]
                        fc = fr_d["cumulative"][li]
                        # Contrarian: extreme positive FR → bearish
                        # extreme negative FR → bullish
                        if abs(fz) > fr_zscore_threshold:
                            if fz > 0 and raw > 0:
                                # Crowded long + our signal long → dampen
                                raw *= max(0.3, 1.0 - fr_weight * (fz - fr_zscore_threshold))
                            elif fz < 0 and raw < 0:
                                # Crowded short + our signal short → dampen
                                raw *= max(0.3, 1.0 - fr_weight * (-fz - fr_zscore_threshold))
                            elif fz > 0 and raw < 0:
                                # Crowded long + our signal short → boost
                                raw *= min(1.5, 1.0 + fr_weight * 0.5)
                            elif fz < 0 and raw > 0:
                                # Crowded short + our signal long → boost
                                raw *= min(1.5, 1.0 + fr_weight * 0.5)
                        # Also: very high cumulative funding → trend exhaustion
                        if abs(fc) > 0.005:  # 0.5% cumulative over 7.5 days
                            if fc > 0 and raw > 0:
                                raw *= 0.7  # longs paying a lot, trend may reverse
                            elif fc < 0 and raw < 0:
                                raw *= 0.7
...
async def main():
    ...
    print("=" * 60, flush=True)
    print(
        "CTA-Forge v12 — Funding Rate Enhanced",
        flush=True,
    )
    print("=" * 60, flush=True)
    ...
    params = {
        ...
        "fr_zscore_threshold": 1.8,
    }

    print("\nLoading funding rate data...", flush=True)
    fr_raw = load_funding_rates()
    fr_aligned = build_fr_lookup(fr_raw, bars)
    print(f"  Funding rates loaded for {len(fr_aligned)} symbols", flush=True)

    print("\nComputing signals...", flush=True)
    sigs = compute_signals(data, timeline, params, fr_aligned=fr_aligned)
    ...
    axes[0].set_title(
        f"CTA-Forge v12 — Funding Rate Enhanced "
    ...
    fig.autofmt_xdate()
    fig.savefig(
        OUT_DIR / "backtest_v12_funding.png",
        dpi=150,
        bbox_inches="tight",
        facecolor="white",
    )
    ...
    # Save metrics
    (OUT_DIR / "metrics_v12_funding.json").write_text(
        json.dumps(
            {
    ...
    print(
        f"Charts: {OUT_DIR / 'backtest_v12_funding.png'}",
        flush=True,
    )
