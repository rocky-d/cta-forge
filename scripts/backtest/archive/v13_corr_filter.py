"""CTA-Forge v13 — v10g + Correlation Filter."""
...
def run_backtest(data, sigs, timeline, start_idx, end_idx, params):
    ...
    CORR_WINDOW = params.get("corr_window", 60)  # 60 bars = 15 days at 6h
    ...
    def get_returns_slice(sym, gt, window):
        """Get recent returns array for correlation calc."""
        start = data[sym]["start_idx"]
        li = gt - start
        if li < window + 1:
            return None
        c = data[sym]["close"]
        rets = np.diff(c[li - window : li + 1]) / c[li - window : li]
        return rets

    def check_correlation(candidate, existing_positions, gt):
        """Check if candidate is too correlated with existing positions."""
        cand_rets = get_returns_slice(candidate, gt, CORR_WINDOW)
        if cand_rets is None:
            return False  # can't compute, allow
        for pos_sym in existing_positions:
            pos_rets = get_returns_slice(pos_sym, gt, CORR_WINDOW)
            if pos_rets is None:
                continue
            if len(cand_rets) != len(pos_rets):
                continue
            # Fast correlation
            cr = np.corrcoef(cand_rets, pos_rets)[0, 1]
            if not np.isnan(cr) and abs(cr) > CORR_THRESHOLD:
        ...
        if t % REBAL == 0 and len(positions) < MAX_POS:
            ...
            for s, sg in cands:
                if len(positions) >= MAX_POS:
                    break
                # Correlation filter
                if positions and check_correlation(s, positions, t):
                    continue
                cp = get_close(s, t)
                a = get_atr(s, t)
                rv = get_rvol(s, t)
...
async def main():
    ...
    print("=" * 60, flush=True)
    print(
        "CTA-Forge v13 — Correlation Filter",
        flush=True,
    )
    print("=" * 60, flush=True)
    ...
    axes[0].set_title(
        f"CTA-Forge v13 — Correlation Filter "
    ...
    fig.autofmt_xdate()
    fig.savefig(
        OUT_DIR / "backtest_v13_corr_filter.png",
        dpi=150,
        bbox_inches="tight",
        facecolor="white",
    )
    ...
    # Save metrics
    (OUT_DIR / "metrics_v13_corr_filter.json").write_text(
        json.dumps(
            {
    ...
    print(
        f"Charts: {OUT_DIR / 'backtest_v13_corr_filter.png'}",
        flush=True,
    )
