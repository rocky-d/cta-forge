"""CTA-Forge v14 — v10g + Dynamic Max Positions + Scaled Entry."""
...
def run_backtest(data, sigs, timeline, start_idx, end_idx, params):
    ...
    MAX_POS_BASE = params["max_positions"]
    MAX_POS_MIN = params.get("max_pos_min", 3)
    MAX_POS_MAX = params.get("max_pos_max", 7)
    ENTRY_SCALE = params.get("entry_scale", 0.6)  # first entry at 60%, add rest on confirmation
    ...
    for t in range(start_idx, end_idx):
        ...
        # Open new positions (dynamic max based on market regime)
        # Compute market-wide average ADX
        adx_vals = []
        for s in data:
            li = t - data[s]["start_idx"]
            if 0 <= li < data[s]["length"]:
                adx_val = data[s]["adx"][li]
                if adx_val > 0:
                    adx_vals.append(adx_val)
        avg_adx = np.mean(adx_vals) if adx_vals else 25
        # Dynamic max positions: high ADX → more, low ADX → fewer
        if avg_adx > 30:
            dyn_max_pos = MAX_POS_MAX
        elif avg_adx < 20:
            dyn_max_pos = MAX_POS_MIN
        else:
            # Linear interpolation between min and max
            dyn_max_pos = int(MAX_POS_MIN + (MAX_POS_MAX - MAX_POS_MIN) * (avg_adx - 20) / 10)

        if t % REBAL == 0 and len(positions) < dyn_max_pos:
            cands = []
            for s in data:
                if s in positions:
                    continue
            ...
            for s, sg in cands:
                ...
                if RISK_PARITY:
                    ...
                    trp = equity * RISK_PT / dyn_max_pos
                    qty_abs = min(
                ...
                qty_abs *= vol_scale * dd_scale * ENTRY_SCALE  # Scaled entry
                if qty_abs * cp < 10:
                    continue
                ...
                positions[s] = {
                    ...
                    "entry_bar": t,
                    "best_price": cp,
                    "scaled": False,  # not yet scaled up
                }

        # Scale up existing positions if signal persists
        if t % REBAL == 0 and ENTRY_SCALE < 1.0:
            for s, po in list(positions.items()):
                if po.get("scaled"):
                    continue
                held = t - po["entry_bar"]
                if held < REBAL:
                    continue  # wait at least one rebalance period
                sig_now = sigs[s][t]
                # Signal still in same direction and strong
                if (po["qty"] > 0 and sig_now >= SIG_T) or \
                   (po["qty"] < 0 and sig_now <= -SIG_T):
                    cp = get_close(s, t)
                    if cp is None:
                        continue
                    # Add the remaining portion
                    add_scale = (1.0 - ENTRY_SCALE) / ENTRY_SCALE
                    add_qty = po["qty"] * add_scale
                    cost = abs(add_qty) * cp + abs(add_qty) * cp * COMMISSION
                    if cash > cost:
                        # Update average entry price
                        old_notional = abs(po["qty"]) * po["entry_price"]
                        new_notional = abs(add_qty) * cp
                        total_qty = po["qty"] + add_qty
                        avg_price = (old_notional + new_notional) / abs(total_qty) if abs(total_qty) > 0 else cp
                        po["qty"] = total_qty
                        po["entry_price"] = avg_price
                        po["scaled"] = True
                        cash -= cost
...
async def main():
    ...
    print("=" * 60, flush=True)
    print(
        "CTA-Forge v14 — Dynamic Positions",
        flush=True,
    )
    print("=" * 60, flush=True)
    ...
    axes[0].set_title(
        f"CTA-Forge v14 — Dynamic Positions "
    ...
    fig.autofmt_xdate()
    fig.savefig(
        OUT_DIR / "backtest_v14_dynpos.png",
        dpi=150,
        bbox_inches="tight",
        facecolor="white",
    )
    ...
    # Save metrics
    (OUT_DIR / "metrics_v14_dynpos.json").write_text(
        json.dumps(
            {
    ...
    print(
        f"Charts: {OUT_DIR / 'backtest_v14_dynpos.png'}",
        flush=True,
    )
