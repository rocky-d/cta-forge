"""CTA-Forge v15c — no max hold + wider stop + asymmetric vol."""
...
def run_backtest(data, sigs, timeline, start_idx, end_idx, params):
    ...
    for t in range(start_idx, end_idx):
        ...
        if TARGET_VOL > 0 and len(recent_returns) >= 20:
            ...
            if rv > 0:
                raw_vs = TARGET_VOL / rv
                # Asymmetric: allow higher vol_scale floor when equity is rising
                if len(recent_returns) >= 20:
                    recent_ret_sum = sum(recent_returns[-20:])
                    if recent_ret_sum > 0:
                        # Equity trending up — don't shrink below 0.6
                        vol_scale = np.clip(raw_vs, 0.6, 2.0)
                    else:
                        vol_scale = np.clip(raw_vs, 0.3, 2.0)
                else:
                    vol_scale = np.clip(raw_vs, 0.3, 2.0)
        dd_scale = (
            0.5
...
async def main():
    ...
    params = {
        ...
        "target_vol": 0.12,
        "btc_filter": True,
        "max_hold_bars": 0,  # disabled — let trailing stop decide
        "tighten_stop_after_atr": 3.5,
        "tightened_stop_mult": 4.0,
    ...
    fig.autofmt_xdate()
    fig.savefig(
        OUT_DIR / "backtest_v15c_combined.png",
        dpi=150,
        bbox_inches="tight",
        facecolor="white",
    )
    ...
    # Save metrics
    (OUT_DIR / "metrics_v15c_combined.json").write_text(
        json.dumps(
            {
    ...
    print(
        f"Charts: {OUT_DIR / 'backtest_v15c_combined.png'}",
        flush=True,
    )
