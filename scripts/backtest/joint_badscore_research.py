"""Research backtest for the current joint v10g + 1h overlay candidate.

Candidate: joint-v10g-fast-overlay-badscore-v1

This script intentionally lives in `scripts/backtest/` as a reproducible research
checkpoint. It is not a production live-trading profile.

Model:
- Core sleeve: shifted v10g 6h trend engine.
- Overlay sleeve: 1h fast-exit top2 engine with a micro pullback risk filter.
- Joint portfolio: engine-derived held position weights, symbol-level netting,
  fixed 50/50 allocation, total gross cap, next-hour mark-to-market.
- Risk gate: fixed badscore2_050; scale to 50% when at least two bad-regime
  conditions are active using past-only expanding thresholds.
...
def load_bars(tf: str, min_bars: int = 500) -> dict[str, object]:
...
def local_overlay_signal(df, data_sym) -> np.ndarray:
    ...
    trend = 0.42 * zret(close, ret1, 24) + 0.35 * zret(close, ret1, 72) + 0.23 * zret(close, ret1, 168)
    ...
    same_side_spike = (np.sign(intrabar) == trend_dir) & (np.abs(intrabar) > 1.5 * np.nan_to_num(rv24, nan=np.inf))
...
def map_local_to_global(local: np.ndarray, start: int, n_global: int, shift: int = 1) -> np.ndarray:
...
def top_n_signals(signals: dict[str, np.ndarray], top_n: int) -> dict[str, np.ndarray]:
    ...
    for t in range(n):
        candidates = [(sym, signals[sym][t]) for sym in syms if np.isfinite(signals[sym][t]) and signals[sym][t] != 0]
        for sym, sig in sorted(candidates, key=lambda x: abs(x[1]), reverse=True)[:top_n]:
            out[sym][t] = sig
    return out
...
def build_overlay_signals(bars, data, timeline) -> dict[str, np.ndarray]:
    ...
    for sym, data_sym in data.items():
        ...
        signals[sym] = map_local_to_global(local, int(data_sym["start_idx"]), len(timeline), shift=1)
    ...
    for t in range(warmup, len(timeline)):
        ...
        for sym, pos in state.positions.items():
            ...
            if snap is None:
                ...
                equity += abs(pos.qty) * pos.entry_price + pos.qty * (snap.close - pos.entry_price)
        ...
        for action in actions:
            ...
            if action.kind in (ActionKind.CLOSE, ActionKind.FLATTEN_ALL):
                ...
                pnl = pos.qty * (price - pos.entry_price) - abs(pos.qty) * price * params.commission
                ...
                pnl = close_qty * (price - pos.entry_price) - abs(close_qty) * price * params.commission
                ...
                state.positions[sym] = PositionState(sym, qty, price, state.bar_count, price)
...
def build_v10g_sleeve():
    ...
    shifted = {sym: np.concatenate([[0.0], np.asarray(sig[:-1], dtype=float)]) for sym, sig in sigs.items()}
...
def align_weights(src_timeline, src_syms, src_weights, dst_timeline, dst_syms, *, forward_fill: bool) -> np.ndarray:
...
def expanding_badscore_gate(timeline) -> np.ndarray:
    ...
    market = np.nanmean(sym_ret, axis=1)
    ...
    for day, vol, eff, corr in zip(days, vol20, eff60, corr60):
        ...
        if len(hist_eff) >= 252 and np.isfinite(eff) and eff < np.nanmedian(hist_eff):
            bad += 1
        if len(hist_corr) >= 252 and np.isfinite(corr) and corr > np.nanquantile(hist_corr, 0.66):
            bad += 1
...
def simulate_joint(ret, v10g_weights, overlay_weights, gate, *, v10g_weight=0.5, overlay_weight=0.5, gross_cap=1.0, fee=0.0004):
    target = v10g_weight * v10g_weights + overlay_weight * overlay_weights
    target *= gate[:, None]
    gross = np.nansum(np.abs(target), axis=1)
    target *= np.where(gross > gross_cap, gross_cap / np.maximum(gross, EPS), 1.0)[:, None]

    pnl = np.zeros(ret.shape[0])
    turnover = np.zeros(ret.shape[0])
    prev = np.zeros(ret.shape[1])
    for t in range(ret.shape[0] - 1):
        cur = np.nan_to_num(target[t], nan=0.0)
        turnover[t] = np.sum(np.abs(cur - prev))
        pnl[t + 1] = np.sum(cur * np.nan_to_num(ret[t + 1], nan=0.0)) - turnover[t] * fee
        prev = cur
    return pnl, turnover, target


def metrics_from_hourly(pnl):
    curve = []
    equity = INITIAL_EQUITY
    for i, ret in enumerate(pnl):
        equity *= 1 + float(ret)
        curve.append((i, equity))
    values = np.array([eq for _, eq in curve])
    total = values[-1] / values[0] - 1
    annualized = (1 + total) ** ((365 * 24) / len(values)) - 1
    vol = np.std(pnl) * np.sqrt(365 * 24)
    sharpe = annualized / vol if vol > EPS else 0.0
    drawdown = values / np.maximum.accumulate(values) - 1
    return {
        "return": total,
        "ann_return": annualized,
        "volatility": vol,
        "sharpe": sharpe,
...
def split_metrics(timeline, pnl):
    ...
    for name, lo, hi in [
        ...
        m = metrics_from_hourly(pnl[mask])
...
def main() -> None:
    t0 = time.time()
    print("Building shifted v10g sleeve...")
    v_syms, v_weights, _v_curve, _v_trades, v_timeline = build_v10g_sleeve()
    print("Building fast-exit 1h overlay sleeve...")
    o_syms, o_weights, _o_curve, _o_trades, o_timeline = build_overlay_sleeve()

    start = max(_v_curve[0][0], _o_curve[0][0])
    end = min(_v_curve[-1][0], _o_curve[-1][0])
    timeline = [ts for ts in o_timeline if start <= ts <= end]
    syms = sorted(set(v_syms) | set(o_syms))

    print("Aligning held position weights...")
    v_aligned = align_weights(v_timeline, v_syms, v_weights, timeline, syms, forward_fill=True)
    o_aligned = align_weights(o_timeline, o_syms, o_weights, timeline, syms, forward_fill=False)
    returns = load_hourly_returns(timeline, syms)
    gate = expanding_badscore_gate(timeline)

    print("Simulating joint candidate...")
    v_only, _, _ = simulate_joint(returns, v_aligned, o_aligned, np.ones(len(timeline)), v10g_weight=1.0, overlay_weight=0.0)
    overlay_only, _, _ = simulate_joint(returns, v_aligned, o_aligned, np.ones(len(timeline)), v10g_weight=0.0, overlay_weight=1.0)
    joint, turnover, target = simulate_joint(returns, v_aligned, o_aligned, gate, v10g_weight=0.5, overlay_weight=0.5)

    series = {
        "shifted_v10g": v_only,
        "fast_exit_top2_overlay": overlay_only,
        "joint_badscore_50_50": joint,
    }
    ...
    result = {
        ...
        "metrics": {k: {mk: mv for mk, mv in v.items() if mk not in {"equity", "drawdown"}} for k, v in metrics.items()},
        "splits": {label: split_metrics(timeline, pnl) for label, pnl in series.items()},
        "avg_gross": float(np.mean(np.sum(np.abs(target), axis=1))),
        "avg_turnover_per_hour": float(np.mean(turnover)),
        "validation_note": "Research approximation using engine-derived held position weights; not a production shared-margin live profile.",
    }
    (OUT_DIR / "metrics_joint_badscore_research.json").write_text(json.dumps(result, indent=2))

    print("\nRESULTS")
    for label, m in metrics.items():
        print(f"  {label}: Sharpe={m['sharpe']:.2f} Return={m['return']*100:+.1f}% MaxDD={m['max_dd']*100:.1f}%")
