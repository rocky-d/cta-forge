...
- v16a target freshness: the 6h v10g core is forward-filled only inside the current 6h window, while the 1h overlay can update hourly between core decisions. Do not forward-fill the core beyond the next scheduled 6h decision time.
